package com.movie_hub.android.ui.main.live.create.adapter;

import android.annotation.SuppressLint;
import android.content.Context;
import android.text.TextUtils;
import android.view.LayoutInflater;
import android.view.ViewGroup;

import androidx.annotation.NonNull;
import androidx.recyclerview.widget.RecyclerView;

import com.bumptech.glide.Glide;
import com.movie_hub.android.R;
import com.movie_hub.android.constant.Constants;
import com.movie_hub.android.data.model.api.response.user.UserResponse;
import com.movie_hub.android.databinding.ItemRoomMemberChipBinding;
import com.movie_hub.android.ui.main.live.create.RoomMemberUiUtils;

import java.util.ArrayList;
import java.util.List;

public class RoomMemberChipAdapter extends RecyclerView.Adapter<RoomMemberChipAdapter.ViewHolder> {

    public interface OnMemberRemoveListener {
        void onMemberRemove(UserResponse user);
    }

    private final List<UserResponse> items = new ArrayList<>();
    private final Context context;
    private final OnMemberRemoveListener listener;

    public RoomMemberChipAdapter(Context context, OnMemberRemoveListener listener) {
        this.context = context;
        this.listener = listener;
    }

    @NonNull
    @Override
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ItemRoomMemberChipBinding binding = ItemRoomMemberChipBinding.inflate(
                LayoutInflater.from(parent.getContext()), parent, false);
        return new ViewHolder(binding);
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        UserResponse item = items.get(position);
        holder.binding.tvName.setText(RoomMemberUiUtils.getChipLabel(item));

        Glide.with(context)
                .load(!TextUtils.isEmpty(item.getAvatarPath())
                        ? Constants.MEDIA_URL + item.getAvatarPath()
                        : null)
                .placeholder(R.drawable.logo)
                .error(R.drawable.logo)
                .into(holder.binding.imgAvatar);

        holder.binding.btnRemove.setOnClickListener(v -> {
            if (listener != null) {
                listener.onMemberRemove(item);
            }
        });
    }

    @SuppressLint("NotifyDataSetChanged")
    public void setData(List<UserResponse> data) {
        items.clear();
        if (data != null) {
            items.addAll(data);
        }
        notifyDataSetChanged();
    }

    @Override
    public int getItemCount() {
        return items.size();
    }

    static class ViewHolder extends RecyclerView.ViewHolder {
        private final ItemRoomMemberChipBinding binding;

        ViewHolder(@NonNull ItemRoomMemberChipBinding binding) {
            super(binding.getRoot());
            this.binding = binding;
        }
    }
}
